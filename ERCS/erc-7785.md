...
Any ENS name MAY resolve to a chain identifier per [ERC-2304](./eip-2304.md). The resolver MUST return the chain identifier. The registry MUST provide methods to retrieve the associated chain data sufficient to recompute and verify the identifier.
...
```solidity
bytes32 node = namehash("rollup.cid.eth")
bytes32 chainId = ChainResolver.resolve(node)
ChainData chainData = ChainRegistry.chainDataFromId()
...
#### CAIP-2 Hashing and Registry Resolution
...
The following pseudocode demonstrates how to resolve a chain from an ERC-7930 address:
...
ChainData chainData = ChainRegistry.chainDataFromCaip2(caip2Hash);
// chainData -> {chainName: string, settlementChainId: uint256, version: string, rollupContract: address, chainNamespace: string, chainReference: string, coinType: uint32}
```
